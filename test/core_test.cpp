...
int main(int argc, char *argv[]) {
    try {
        ...
        //  加载工况
        //=======================
        std::vector<std::string> cases = {"wigleyI",   "wigleyI_Lu", "S175_1424", "S175_1262", "S175_924",  "S175_848",
                                          "S175_1722", "S175_1476",  "S175_1874", "S175_3418", "S175_18016"};
        ...
        if (cfg.enable_seakeeping && cfg.Seakeeping.has_value()) {
            ...
            if (!std::filesystem::is_regular_file(casePath + shipname + ".element")) {
                if (std::filesystem::is_regular_file(casePath + shipname + ".dat"))
                    CaseLoader::UGtoElement(casePath + shipname + ".dat", casePath + shipname + ".element", 0.001);
                else
        ...
        if (cfg.enable_coupling && cfg.Coupled.has_value()) {
            ...
            if (cfg.Coupled->physics.enableFastSeakeeping) {
                ...
                radRepo = std::make_unique<CoupledRadiationKernelRepo>(cfg.Ship, cfg.Seakeeping.value(), casePath);
                ...
                // TDGF solver and for direct-pressure FK wave force.
                {
                    ...
                    auto elementData =
                        CaseLoader::loadelement(elementFile, cfg.Seakeeping->Panel.NEType, cfg.Seakeeping->Panel.NE);
                    coupledElement = std::make_shared<Element>(cfg.Seakeeping->Solver, cfg.Seakeeping->Panel.NE,
                                                               std::move(elementData));
                    Eigen::Vector3d cg(cfg.Ship.Mass.CG.at(0), cfg.Ship.Mass.CG.at(1), cfg.Ship.Mass.CG.at(2));
                ...
                if (mode == "fitted") {
                    ...
                    if (mode == "directFK") {
                        ...
                        waveForceProvider = std::make_unique<DirectFKPlusDfWaveForceProvider>(
                            cfg.Ship, cfg.Seakeeping.value(), casePath, cfg.Ship.Name, cfg.Coupled->refresh,
                            coupledElement);
                        ...
                        excRepo =
                            std::make_unique<CoupledExcitationKernelRepo>(cfg.Ship, cfg.Seakeeping.value(), casePath);
                    ...
                    fastSolver = std::make_unique<TDGFWindowSeakeepingSolver>(
                        cfg.Ship, cfg.Seakeeping.value(), casePath, *radRepo, cfg.Coupled->refresh, *waveForceProvider,
                        coupledElement, cfg.Coupled->physics.enableRadiation);
                }
            } else {
            ...
            TwoTimeScaleCoordinator coordinator(cfg, casePath, *fastSolver, *lowFreqSolver, *secondOrderProvider,
                                                waveEnv);
